...
### Redondance — ce qui manque

La configuration actuelle stocke les backups sur la même carte SD que le système. C'est une **configuration minimale** — si la carte SD lâche, on perd à la fois le système et les backups. En production, c'est inacceptable.

La règle de base en backup c'est la **règle 3-2-1** :
- **3** copies des données
- sur **2** supports différents
- dont **1** hors site (distant)

| Niveau | Solution | Statut |
|--------|----------|--------|
| Backup local sur SD | Ce qu'on a mis en place | ✅ En place |
| Backup sur disque externe USB | Disque branché au Pi | ❌ Matériel non disponible |
| Backup distant hors site | NAS, cloud (Backblaze B2, S3...) | ❌ Matériel/abonnement non disponible |

**Ce qu'il faudrait mettre en place dès que le matériel est disponible :**

```yaml
# Ajouter un deuxième dépôt dans config.yaml
repositories:
    - path: /backup/borg
      label: local

    # Disque externe USB monté sur /mnt/usb
    - path: /mnt/usb/borg
      label: usb

    # Ou vers un serveur distant via SSH
    - path: ssh://user@serveur-distant/backup/borg
      label: remote
```

Borgmatic sauvegarde automatiquement vers tous les dépôts configurés à chaque exécution — il suffit d'ajouter les entrées dans `config.yaml`.

> ⚠️ Dans l'état actuel, si la carte SD est corrompue ou volée avec le Pi, toutes les données sont perdues. Cette configuration est suffisante pour un homelab/portfolio, mais ne doit pas être utilisée pour des données critiques sans redondance.
